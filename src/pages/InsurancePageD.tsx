import { CategoryPageD, formatNIS, type CategoryItem } from "./CategoryPageD";
import { insuranceItems } from "@/lib/data";

const InsurancePageD = () => {
  ...
  const yearlyFromYearly = activePolicies
    ...
    .reduce((s, i) => s + (i.cost ?? 0), 0);
  const totalMonthly = Math.round(monthlyFromMonthly + yearlyFromYearly / 12);
  ...
  return (
    <CategoryPageD
      ...
      theme={{
        gradient:
          "radial-gradient(ellipse 90% 70% at 50% 0%, hsl(275, 80%, 38%) 0%, hsl(265, 60%, 18%) 50%, hsl(222, 45%, 7%) 100%)",
        accent: "hsl(280, 90%, 70%)",
        accentBg: "hsla(275, 70%, 30%, 0.35)",
        accentText: "hsl(280, 90%, 78%)",
        sheetShadow: "hsla(275, 90%, 40%, 0.45)",
      }}
      items={items}
      filters={[
      ...
      renderItemTrailing={(item) => {
        ...
        const monthlyCost =
          item.billing === "monthly" ? item.cost ?? 0 : Math.round((item.cost ?? 0) / 12);
        ...
        return (
          <span
            ...
            style={{
              ...
              color: "hsl(0, 90%, 78%)",
              border: "1px solid hsla(0, 90%, 60%, 0.4)",
            }}
          >
            חסר
          </span>
